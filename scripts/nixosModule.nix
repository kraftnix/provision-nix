...
in {
  options.provision.scripts = {
    ...
    addToSystemPackages = opts.enableTrue "add all enabled scripts to `environment.systemPackages`";
  ...
  config = lib.mkIf cfg.enable {
    environment.systemPackages = mkIf cfg.addToSystemPackages (mapAttrsToList (_: c: c.package) cfg.__enabledScripts);
  };
}
