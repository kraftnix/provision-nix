...
rustPlatform.buildRustPackage (
  let
    ...
    pname = "nushell";
    version =
      if builtins.stringLength source.version > 8
      then builtins.substring 0 8 source.version
      else source.version;
  in {
    ...
    buildInputs =
      [openssl zstd]
      ++ lib.optionals stdenv.isDarwin [zlib libiconv]
      ++ lib.optionals (withDefaultFeatures && stdenv.isLinux) [xorg.libX11]
      ++ lib.optionals (withDefaultFeatures && stdenv.isDarwin) [nghttp2 libgit2];
