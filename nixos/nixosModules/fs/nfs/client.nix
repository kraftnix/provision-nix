{self, ...}: {
  ...
  inherit (lib) filterAttrs flatten mapAttrs' mkIf mkOption nameValuePair optionals pipe types unique;
  opts = self.lib.options;
  cfg = config.provision.fs.nfs.client;
  mkServiceList = default: description:
    mkOption {
    ...
    mounts = mkOption {
      ...
      type = types.attrsOf (types.submoduleWith {
        ...
        modules = [
          ...
          {
            config._module.args = {
              inherit (cfg) localBase remoteBase remoteUrl nfsVersion;
            };
          }
        ];
      });
