  ...
  options.provision.networking.wireguard.p2p.hosts = mkOption {
    ...
    type = types.attrsOf (types.submodule ({
      ...
      options = {
        ...
        networks = mkOption {
          default = {};
          type = types.attrsOf (types.submodule (n @ {name, ...}: {
            options = {
              enable = opts.enable' (n.config.pubkey != "") "enable host in wireguard network, enabled if pubkey set";
        ...
        _: host:
          ...
          (_: network: {
            ${network.name}.peers.${host.name} = {
              inherit (network) subip pubkey mtu endpointIP extraAllowedIPs;
