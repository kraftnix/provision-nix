...
in {
  options.perSystem = flake-parts-lib.mkPerSystemOption ({pkgs, ...}: {
    ...
    options.nuscht-search = mkOption {
      ...
      type = types.attrsOf (types.submodule ({
        ...
        options = {
          enable = mkEnableOption "enable nuscht-search generation" // {default = true;};
  ...
  config.perSystem = {config, ...}: {
    packages = mapAttrs' (name: search: nameValuePair "nuscht-search-${name}" search.multiSearch) config.nuscht-search;
  };
}
