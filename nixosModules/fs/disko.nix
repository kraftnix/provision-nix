{self, ...}: {
  ...
  inherit (lib) filterAttrs hasPrefix mapAttrsToList mkIf mkMerge mkOption pipe recursiveUpdate types;
  ...
  options.provision.fs.disko = {
    ...
    devices = lib.mkOption {
      ...
      type = types.attrsOf (types.submodule ({config, ...}: {
        ...
        config.generated = mkIf (config.profile != "") (pipe config.args [
          (filterAttrs (name: _: !(hasPrefix "__" name)))
          (filtered:
            import config.__profilePath (filtered
              // {
                inherit lib;
              }))
        ]);
