{
  ...
  chainTypeModule = types.submodule ({config, ...}: {
    options = {
      type = mkOption {
        description = ''
      ...
      hook = mkOption {
        ...
        '';
        type = types.enum ["ingress" "prerouting" "forward" "input" "output" "postrouting" "egress"];
        default = "filter";
      };
      priority = mkOption {
        description = ''
      ...
      policy = mkOption {
        ...
        '';
        type = with types; nullOr (enum ["accept" "drop"]);
        default = null;
      };
      __rendered = mkOption {
  ...
  options = {
    __type = mkOption {
      description = ''
        ...
        https://wiki.nftables.org/wiki-nftables/index.php/Quick_reference-nftables_in_10_minutes#Chains
      '';
      type = with types; oneOf [str chainTypeModule];
      default = "";
      apply = v:
    ...
    rules = mkOption {
      description = "Rules";
      type = with types; attrsOf (oneOf [str finalRuleModule]);
      default = {};
      apply = mapAttrs (_: v:
        if builtins.typeOf v == "string"
        then
